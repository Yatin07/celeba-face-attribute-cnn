...
def predict_image(model, image_path, threshold=0.4, show_top=10):
    ...
    print(f'\n🌟 TOP {show_top} HIGHEST CONFIDENCE PREDICTIONS:')
    print(f'{"#":<4} {"Attribute":<25} {"Confidence":>12} {"Present?"}')
    print('-'*70)
    
    for i, r in enumerate(results_sorted[:show_top], 1):
...
def main():
    parser = argparse.ArgumentParser(description='Predict facial attributes on your photo')
    parser.add_argument('image', type=str, help='Path to your image (jpg, png, etc.)')
    parser.add_argument('--model', type=str, choices=['best', 'last'], default='best',
                        help='Which model to use: best or last (default: best)')
    parser.add_argument('--checkpoint', type=str, default=None,
                        help='Path to custom checkpoint file')
    parser.add_argument('--threshold', type=float, default=0.4,
                        help='Prediction threshold (default: 0.4)')
    ...
    # Predict
    results = predict_image(model, args.image, threshold=args.threshold, show_top=args.top)
