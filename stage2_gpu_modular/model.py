...
#   Block 1: Conv(3→64)   + BN + ReLU + MaxPool → (B, 64, 64, 64)
#   Block 2: Conv(64→128)  + BN + ReLU + MaxPool → (B, 128, 32, 32)
#   Block 3: Conv(128→256) + BN + ReLU + MaxPool → (B, 256, 16, 16)
#   Block 4: Conv(256→256)+ BN + ReLU + MaxPool → (B, 256,  8,  8)
#   GlobalAvgPool → (B, 256)
...
#
# Params: ~2M  (safe for GPU, improved learning capacity)
# ============================================================
...
class SimpleCNN(nn.Module):
    ...
    def __init__(self, num_classes=40, dropout=0.4):
        ...
        self.block1 = _conv_block(3,   64)   # 128×128 → 64×64
        self.block2 = _conv_block(64,  128)  #  64×64  → 32×32
        self.block3 = _conv_block(128, 256)  #  32×32  → 16×16
        self.block4 = _conv_block(256, 256)  #  16×16  →  8×8
...
def model_summary(model):
    ...
    x = model.block1(x); print(f"  Block1 : {tuple(x.shape)}  [Conv 3->64]")
    x = model.block2(x); print(f"  Block2 : {tuple(x.shape)}  [Conv 64->128]")
    x = model.block3(x); print(f"  Block3 : {tuple(x.shape)}  [Conv 128->256]")
    x = model.block4(x); print(f"  Block4 : {tuple(x.shape)}  [Conv 256->256]")
